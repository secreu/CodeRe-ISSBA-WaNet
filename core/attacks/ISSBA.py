...
class ISSBA(Base):
    ...
    def train(self, schedule=None):
        ...
        for i in range(self.current_schedule['epochs']):
            ...
            # plt.subplot(1, 2, 1)
            # plt.plot(train_steps, losses, label='Loss')
            # plt.xlabel('Epochs')
            # plt.ylabel('Loss')
            # plt.legend()

            # plt.subplot(1, 2, 2)
            # plt.plot(train_steps, cl_accuracies, label='Clean Accuracy')
            # plt.plot(train_steps, bd_accuracies, label='Poisoned Accuracy')
            # plt.xlabel('Epochs')
            # plt.ylabel('Accuracy')
            # plt.legend()
            ...
            # plt.tight_layout()
            # plt.show()
            # # plt.savefig("Results\\ISSBA\\train.jpg")
            ...
            if (i + 1) % self.current_schedule['test_epoch_interval'] == 0:
                ...
                cl_accuracies.append(top5_correct/total_num)
                ...
                bd_accuracies.append(top5_correct/total_num)
        ...
        plt.tight_layout()
        # plt.show()
        plt.savefig("Results\\ISSBA\\issba_train.jpg")
    ...
    def _test(self, dataset, device, batch_size=16, num_workers=0, model=None):
        if model is None:
            model = self.model
        else:
            model = model
